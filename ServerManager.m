...
+ (void)sendRequestForLogin:(NSString*)userName password:(NSString*)password language:(NSString*)language  onSucess:(void (^)(NSString *responseData))success onFailure:(void (^) (NSString *responseData, NSError *error))failure
{
    @try{
        //
        ...
        [NSURLConnection sendAsynchronousRequest:request queue:[NSOperationQueue mainQueue] completionHandler:^(NSURLResponse *response, NSData *data, NSError *connectionError) {
            @try {
                ...
                if(!connectionError){
                    ...
                    strServerErr  = NSLocalizedString(@"Server Error." , @"") ;
                    strTimeout = NSLocalizedString(@"Request Timeout error." , @"");
                    
                    NSArray* resultArray1 = [[CoreDataHandler sharedHandler] getTranslatedText:[[NSMutableArray alloc] initWithObjects:NSLocalizedString(@"Request Timeout error." , @""),NSLocalizedString(@"Server Error.",@""),nil]];
...
+ (void)sendRequest:(NSString*)url idOfServiceUrl:(NSInteger)idOfServiceUrl headers:(NSMutableDictionary*)headers methodType:(NSString*)methodType onSucess:(void (^)(NSString *responseData))success onFailure:(void (^) (NSString *responseData, NSError *error))failure{
    @try {
...
//                }
                ...
                //
                NSString *strUnauthorised,*strServerErr,*strTimeout,*strConnectTimeout,*strReadTimeout,*strGatewaytimeout;
